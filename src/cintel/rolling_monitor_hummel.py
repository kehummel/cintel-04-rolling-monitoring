...
Paths (relative to repo root)
    ...
    INPUT FILE: data/system_metrics_timeseries_hummel.csv
    OUTPUT FILE: artifacts/rolling_metrics_hummel.csv

Terminal command to run this file from the root project folder

    uv run python -m cintel.rolling_monitor_hummel
...
DATA_FILE: Final[Path] = DATA_DIR / "system_metrics_timeseries_hummel.csv"
OUTPUT_FILE: Final[Path] = ARTIFACTS_DIR / "rolling_metrics_hummel.csv"
...
def main() -> None:
    ...
    errors_rolling_mean_recipe: pl.Expr = (
        pl.col("errors").rolling_mean(WINDOW_SIZE).round(2).alias("errors_rolling_mean")
    )
